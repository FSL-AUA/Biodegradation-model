ANIPH Machine Learning Model

This repository contains the machine learning workflow developed in the ANIPH project for predicting biodegradation percentage of PHBV polymers.

This folder includes:

1) Original dataset (*_Dataset.xlsx)

2) Preprocessing notebook (*_data_preprocess_LM.ipynb)

3) Cleaned dataset (*_data_LM.csv) Dataset obtained after preprocessing (cleaning, filtering, unit harmonization, removing invalid samples).

4) Final training and deployment on Jaqpot notebook (*_regression_LM.ipynb)


The deployed model was trained and optimized using Random Forest–based techniques tailored to the available experimental data.
